...
file_tups_traj = [['traj_cychex_neat/Lang_1_baro1step/cyclohexane_250_'+i+'_wHConstraints_1fs.nc'] for i in file_str]
file_tups_traj_vac = [['traj_cychex_neat/Lang_1_baro1step/cyclohexane_'+i+'_wHConstraints_vacuum.nc'] for i in file_str]

file_tups_sd = [['StateData_cychex_neat/Lang_1_baro1step/cyclohexane_250_'+i+'_wHConstraints_1fs.csv'] for i in file_str]
file_tups_sd_vac = [['StateData_cychex_neat/Lang_1_baro1step/cyclohexane_'+i+'_wHConstraints_vacuum.csv'] for i in file_str]
...
#    E_kn_294, u_kn_294 = new_param_energy(xyz_sub[ii], D_mol, pdb.topology, vecs_orig_sub[ii], T = 294.15)
    ...
    betapV = (1./(kB*T))*P*np.array(vol_box_sub)*J2kJ*A32m3*N_Av
    u_kn += betapV
    ...
    df.to_csv('MBAR_estimates_[6X4:1]_eps'+argv[1]+'-'+argv[2]+'_rmin'+argv[3]+'-'+argv[4]+'_baro10step_directboxcalcs_wHConstraints_MBAR1e-5PMEco_Lang_1fs_1nsburn.csv',sep=';')
