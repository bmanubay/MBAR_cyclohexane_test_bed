...
files = glob.glob('cyclohexane_250_*1fs.csv')
file_strings = [i.rsplit('.',1)[0].rsplit('_',1)[0].split('_',2)[2] for i in files]
print(file_strings)
file_str = set(file_strings)

file_tups_sd = [['cyclohexane_250_'+i+'_1fs.csv'] for i in file_str]
file_tups_sd_vac = [['cyclohexane_'+i+'_vacuum.csv'] for i in file_str]
print(file_tups_sd)
...
for n in range(nBoots_work):
    ...
    Cp_boots.append(Cp_bulk_sim)
    V_boots.append([np.mean(j) for j in vol_orig_sub_boot])
Cp_boots_vert = np.column_stack(Cp_boots)
V_boots_vert = np.column_stack(V_boots)
Cp_bulk_sim = Cp_boots[0]
V_sim = V_boots[0]
dCp_bulk_bootstrap = [np.std(i) for i in Cp_boots_vert]
...
for n in range(nBoots_work):
    ...
    Cp_vac_boots.append(Cp_vac_sim)
Cp_vac_boots_vert = np.column_stack(Cp_vac_boots)
Cp_vac_sim = Cp_vac_boots[0]
dCp_vac_bootstrap = [np.std(i) for i in Cp_vac_boots_vert]

Cp_res_sim = [bulk - vac for bulk,vac in zip(Cp_bulk_sim,Cp_vac_sim)]
dCp_res_sim = [np.sqrt(bulk**2 + gas**2) for bulk,gas in zip(dCp_bulk_bootstrap,dCp_vac_bootstrap)]

Vol_sim = V_sim
dVol_sim = dV_bootstrap

eps = [float(i[0]) for i in params]
rmin = [float(i[1]) for i in params]
print(eps)
print(len(eps))
print(rmin)
print(len(rmin))
...
df = pd.DataFrame(
                          {'eps_val': eps,
                           ...
                           'C_p_res_sim (J/mol/K)': Cp_res_sim,
                           'dC_p_res_sim (J/mol/K)': dCp_res_sim
                          })

df.to_csv('cychex_simulated_observables_partial_baro10step_molvolcalcs_wHConstraints_1fs_1nsburn_Cpmeth<E2>-<E>2.csv',sep=';')
exit()
